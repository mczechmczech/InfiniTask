...
def on_click_new_sort():
    ...
    # query the DB, sort the results
    sql_query = "SELECT * FROM Job ORDER BY " + new_sort + " ASC;"
    cursor.execute(sql_query)
...
def on_click_login():
    ...
    sql_query = "SELECT Password FROM Login WHERE Username = \"" + username + "\";"
    cursor.execute(sql_query)
    password_result = cursor.fetchone()
    if password_result is not None:
        password_result_edit = "".join(x for x in password_result if x.isalpha())

        if password_result_edit == password:
            valid_login = "True"
    else:
        valid_login = "False"
...
def main():
    ...
    if valid_login:
        ...
        Button(main_window, text="Enter New Job", command=on_click_new_job, anchor=W).grid(sticky=W, row=2, column=0,
                                                                                           columnspan=2)
